    ...
    public class TestService : ITestService
    {
        ...
        public async Task SHORT_Deep()
        {
            try
            {
                ...
                var symbols = lSym.Select(x => x.s).ToList();
                    ...
                    try
                    {
                        ...
                        foreach (var ma20 in lbb)
                        {
                            try
                            {
                                ...
                                float avgMA20 = 0, avg1_3_UP = 0;
                                foreach (var val in dic.Values)
                                {
                                    var first = val.First(x => x.Date > entity_Sig.Date);
                                    if (first.TotalRateMa20_UP == 1)
                                        avgMA20++;

                                    if (first.TotalRate1_3_UP == 1)
                                        avg1_3_UP++;
                                }
                                double rateMA20 = Math.Round(100 * avgMA20 / dic.Count());
