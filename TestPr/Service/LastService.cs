...
namespace TestPr.Service
{
    ...
    public class LastService : ILastService
    {
        ...
        public async Task fake2()
        {
            ...
            var lbb = quotes.GetBollingerBands(20, 2).ToList();
                ...
                if (up > 1.3m)
                {
                        ...
                        && rsi.Rsi.Value < ma9.Sma.Value)
                    {
                        ...
                        foreach(var item in lEntry)
                        {
                            ...
                            var rsi_check = lrsi.First(x => x.Date == item.Date);
                            var ma9_check = lma9.First(x => x.Date == item.Date);
                            if(rsi_check.Rsi.Value > ma9_check.Sma.Value)
                            {
                                var bb_check = lbb.Where(x => x.Date < item.Date).TakeLast(20).MaxBy(x => x.UpperBand - x.LowerBand);
                                var bb_cur = lbb.First(x => x.Date == item.Date);
                                var rate = Math.Round(100 * (-1 + bb_check.UpperBand.Value / bb_check.LowerBand.Value), 2);
                                //rate >= 10 hoặc close > ma20 
                                if(rate >= 10
                                    || item.High > (decimal)bb_cur.Sma.Value)
                                {
                                    //Console.WriteLine($"==================>LOAI: {item.Date.ToString("dd/MM HH:mm")}");
                                    break;
                                }
        ...
        decimal SimulateTrade2(Pro entry, int index, List<Quote> futureQuotes)
        {
            ...
            for (int i = index + 1; i < futureQuotes.Count(); i++)
            {
                ...
                if (q.Low <= entry.sl_price)
                {
                    ...
                    _cap -= loss;
                    Console.WriteLine($"{entry.entity.Date.ToString("dd/MM/yyyy HH")}|{q.Date.ToString("dd/MM/yyyy HH")}|BALANCE: {_cap}|STOPLOSS: {-loss}");
                    return 0;
                }
                //dong lenh
                if(futureQuotes.Count(x => x.Date > entry.entity.Date && x.Date <= q.Date) >= SONEN_NAMGIU)
                {
                    ...
                    _cap += rate;
                    Console.WriteLine($"{entry.entity.Date.ToString("dd/MM/yyyy HH")}|{q.Date.ToString("dd/MM/yyyy HH")}|BALANCE: {_cap}|TiLe: {Math.Round(tile * 100, 2)}|EXPIRED: {rate}");
                    return 0;
                }    
                    ...
                    && lrsi[i].Rsi < lma9[i].Sma)
                {
                    ...
                    _cap += win;
                    Console.WriteLine($"{entry.entity.Date.ToString("dd/MM/yyyy HH")}|{q.Date.ToString("dd/MM/yyyy HH")}|BALANCE: {_cap}|TiLe: {Math.Round(tile * 100, 2)}|TP3: {win}");
                    return 1;
                }
            }
            //return profit;
