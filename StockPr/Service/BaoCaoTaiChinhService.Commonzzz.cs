...
namespace StockPr.Service
{
    ...
    {
        private async Task SyncBCTCzz(bool onlyLast = false)
        {
            try
            {
                ...
                {
                    await SyncBCTC_KQKDzz(item, onlyLast);
                    await SyncBCTC_CDKTzz(item, ECDKTType.Normal);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task SyncBCTC_BatDongSanzz(bool onlyLast = false)
        {
            try
            {
                ...
                {
                    await SyncBCTC_KQKDzz(item, onlyLast);
                    await SyncBCTC_CDKTzz(item, ECDKTType.BDS);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task SyncBCTC_KQKDzz(string code, bool onlyLast)
        {
            try
            {
        ...
        private async Task SyncBCTC_CDKTzz(string code, ECDKTType type)//type: 1-Normal
        {
            try
            {
                var time = GetCurrentTime();
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        {
                            entityUpdate = CDKT_BDSzz(entityUpdate, lData, i);
                        }
                        else if (type == ECDKTType.ChungKhoan)
                        {
                            entityUpdate = CDKT_ChungKhoanzz(entityUpdate, lData, i);
                        }
                        else
                        {
                            entityUpdate = CDKT_Normalzz(entityUpdate, lData, i);
                        }
                        _financialRepo.Update(entityUpdate);
                    }
                }
        ...
        private Financial CDKT_Normalzz(Financial entityUpdate, ReportDataDetailValue_BCTTResponse lData, int i)
        {
            try
            {
                entityUpdate.t = (int)DateTimeOffset.Now.ToUnixTimeSeconds();
        ...
        private Financial CDKT_BDSzz(Financial entityUpdate, ReportDataDetailValue_BCTTResponse lData, int i)
        {
            try
            {
                entityUpdate.t = (int)DateTimeOffset.Now.ToUnixTimeSeconds();
        ...
        private Financial CDKT_ChungKhoanzz(Financial entityUpdate, ReportDataDetailValue_BCTTResponse lData, int i)
        {
            try
            {
                entityUpdate.t = (int)DateTimeOffset.Now.ToUnixTimeSeconds();
